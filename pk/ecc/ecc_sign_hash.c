...
/**
  Sign a message digest for fast verification
  @param in        The message digest to sign
  @param inlen     The length of the digest
  @param out       [out] The destination for the signature
  @param outlen    [in/out] The max size and resulting size of the signature
  ...
  @return CRYPT_OK if successful
*/
int ecc_sign_hash_fast(const unsigned char *in,  unsigned long inlen, 
                        unsigned char *out, unsigned long *outlen, 
                        prng_state *prng, int wprng, ecc_key *key)
{
   ecc_key       pubkey;
   ...
   /* get the hash and load it as a bignum into 'e' */
   /* init the bignums */
   if ((err = mp_init_multi(&Rx, &Ry, &r, &s, &p, &e, NULL)) != CRYPT_OK) { 
      return err;
   }
   if ((err = mp_read_radix(p, (char *)key->dp->order, 16)) != CRYPT_OK)                      { goto errnokey; }
   if ((err = mp_read_unsigned_bin(e, (unsigned char *)in, (int)inlen)) != CRYPT_OK)          { goto errnokey; }
   ...
   for (;;) {
      ...
      if (mp_iszero(r) == LTC_MP_YES) {
        ...
        if ((err = mp_copy(pubkey.pubkey.x, Rx)) != CRYPT_OK)                { goto error; }
        if ((err = mp_copy(pubkey.pubkey.y, Ry)) != CRYPT_OK)                { goto error; }
   ...
   /* store as SEQUENCE { Rx, Ry, r, s -- integer } */
   err = der_encode_sequence_multi(out, outlen,
...
error:
   ecc_free(&pubkey);
errnokey:
   mp_clear_multi(Rx, Ry, r, s, p, e, NULL);
   return err;   
}
