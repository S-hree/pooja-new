 ...
 * =================================================
 *
 * In the general case, the doubling of P = (Xp, Yp, Zp) is given by
 * R = (Xr, Yr, Zr) such that
 *                    Xr = M^2 - 2*S
 *                    Yr = M*(S - Xr) - 8*T
 *                    Zr = 2*Yp*Zp
 * where
 ...
 *                    T = Yp^4
 *                    S = 4*Xp*Yp^2
 * In the special case where a == -3, M can be computed more efficiently as
 *          M = 3*(Xp^2 - Zp^4) = 3*(Xp + Zp^2)*(Xp - Zp^2)
 *
 * References:
 *   - "Guide to Elliptic Curve Cryptography", by Darrel Hankerson,
 *     Alfred J. Menezes, Scott Vanstone, Springer, 2004:
 *     Sections 3.2.1, 3.2.2 and Example 3.20 
 *   - "Cryptography for Developers" by Tom St Denis, Syngress, 2007:
 *     Chapter 9, section "Elliptic Curve Performance".
 */
...
/**
   ...
   @param a   ECC domain parameter 'a' (if not specified, we assume a == -3)
