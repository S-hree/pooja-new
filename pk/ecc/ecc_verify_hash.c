                    ...
                    int *stat, ecc_key *key)
{
   ...
   *stat = 0;
   ...
   if (ltc_mp.ecc_mul2add == NULL) {
      if ((err = ltc_mp.ecc_ptmul(u1, mG, mG, key->dp->A ? a : NULL, m, 0)) != CRYPT_OK)                { goto error; }
      if ((err = ltc_mp.ecc_ptmul(u2, mQ, mQ, key->dp->A ? a : NULL, m, 0)) != CRYPT_OK)                { goto error; }
  
      /* find the montgomery mp */
      if ((err = mp_montgomery_setup(m, &mp)) != CRYPT_OK)                                              { goto error; }

      /* add them */
      if ((err = ltc_mp.ecc_ptadd(mQ, mG, mG, key->dp->A ? a : NULL, m, mp)) != CRYPT_OK)               { goto error; }
   
      /* reduce */
      if ((err = ltc_mp.ecc_map(mG, m, mp)) != CRYPT_OK)                                                { goto error; }
   } else {
      /* use Shamir's trick to compute u1*mG + u2*mQ using half of the doubles */
      if ((err = ltc_mp.ecc_mul2add(mG, u1, mQ, u2, mG, key->dp->A ? a : NULL, m)) != CRYPT_OK)         { goto error; }
   }
