...
//#define TX_DISABLE_ERROR_CHECKING /* AT-5010 && AT-4925 */
#endif
#endif
...
#define NX_PHYSICAL_HEADER          (16 + NX_CACHE_LINE_SIZE)                /* Maximum physical header        */
#endif

#ifndef NX_PHYSICAL_TRAILER
#define NX_PHYSICAL_TRAILER         (4 + NX_CACHE_LINE_SIZE)                  /* Maximum physical trailer       */
#endif
...
#define NX_RAW_PACKET         NX_PHYSICAL_HEADER
...
/* Define the maximum number of the packets that can be queued for IP raw receive. */
...
#ifndef NX_RAW_MAX_QUEUE_DEPTH   
#define NX_RAW_MAX_QUEUE_DEPTH   20
#endif
...
#define NX_LINK_RAW_PACKET_SEND     51
#define NX_LINK_RAW_PACKET_ROUTE_IP_ENABLE  52
#define NX_LINK_RAW_PACKET_ROUTE_IP_DISABLE 53
...
#define nx_ip_additional_link_info     nx_ip_interface[0].nx_interface_additional_link_info
    ...
    ULONG       nx_raw_packets_received;
    ULONG       nx_raw_bytes_received;
    ULONG       nx_raw_receive_packets_dropped;

    ULONG       nx_raw_packets_sent;
    ULONG       nx_raw_bytes_sent;
    ULONG       nx_raw_send_packets_dropped;
    ...
    /* Define the raw function pointer that also indicates whether or 
       not raw packet sending and receiving is enabled.  */
    UINT        (*nx_raw_processing)(struct NX_IP_STRUCT *, NX_PACKET *);
    ...
    /* Define the pointer to the raw packet queue.  */
    NX_PACKET   *nx_raw_received_packet_head,
                *nx_raw_received_packet_tail;

    /* Define the count of raw packets on the queue.  */
    ULONG       nx_raw_received_packet_count;

    /* Define the maximum number of packets queued for receive. */
    ULONG       nx_raw_received_packet_max;

    /* Define the raw packet suspension list head along with a count of
       how many threads are suspended.  */
    TX_THREAD   *nx_raw_packet_suspension_list;
    ULONG       nx_raw_packet_suspended_count;
...
#define nx_raw_packet_disable                   _nx_raw_packet_disable
#define nx_raw_packet_enable                    _nx_raw_packet_enable
#define nx_raw_packet_ip_route_enable           _nx_raw_packet_ip_route_enable
#define nx_raw_packet_receive                   _nx_raw_packet_receive
#define nx_raw_packet_send                      _nx_raw_packet_send
...
UINT    nx_raw_packet_disable(NX_IP *ip_ptr);
UINT    nx_raw_packet_enable(NX_IP *ip_ptr);
UINT    nx_raw_packet_receive(NX_IP *ip_ptr, UINT interface_index, NX_PACKET **packet_ptr, ULONG wait_option);
UINT    nx_raw_packet_send(NX_IP *ip_ptr, NX_PACKET *packet_ptr, UINT interface_index);
UINT    nx_raw_packet_ip_route_enable(NX_IP *ip_ptr,
                                      INT enabled,
...
/* Define the direct raw packet receive processing.  This is the lowest overhead way
   to notify NetX of a received raw packet, however, it results in the most amount of
   processing in the driver's receive ISR.  If the driver deferred packet processing
   is used, this routine should be used to notify NetX of the newly received IP packet.  */

VOID  _nx_raw_packet_receive_from_driver(NX_IP *ip_ptr, NX_PACKET *packet_ptr);
