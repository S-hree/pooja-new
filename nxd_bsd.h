...
/*  06-30-2013     Yuxin Zhou               Modified comment(s), and      */ 
/*                                            fixed FD_SETSIZE value,     */
/*                                            resulting in version 5.3-sp1*/
/*  09-20-2013     Yuxin Zhou               Modified comment(s), added    */
/*                                            PPPoE raw packet support,   */
/*                                            resulting in version 5.3-sp2*/
...
/*                                            timeout option NX_BSD_TCP_  */
/*                                            _SOCKET_DISCONNECT_TIMEOUT, */
/*                                            resulting in version 5.3-sp3*/
/*                                                                        */ 
/**************************************************************************/
...
/* Bring in errno codes */
#include <sys/errno.h>
#include <fcntl.h>

/* Bring in struct timeval definition */
#include <sys/time.h>

/* Bring in the necessary NetX include file.  */
#include "nx_api.h"

/* [Autotalks] We're using sys/time.h instead */
#define __NX_BSD_DONT_DEFINE_TIMEVAL__

/* [Autotalks] We want to override default socket timeout to
   effectively block forever */
#define NX_BSD_TIMEOUT (1 << 31)

/* [Autotalks] Timeout for allocating data for NX BSD socket */
/* 2 seconds is an arbitrary chosen value for NX packet allocation timeout */
#define NX_BSD_PACKET_ALLOCATE_TIMEOUT_MS (2000)
...
#define NX_CPU_TICKS_PER_SECOND             1000                     /* Number of timer ticks per second.                                   */  
#endif

#ifndef NX_MICROSECOND_PER_CPU_TICK
#define NX_MICROSECOND_PER_CPU_TICK         1000                    /* Number of microseconds per timer interrupt.                          */ 
#endif
...
#define NX_BSD_SOCKET_QUEUE_MAX 32
#endif
...
#ifndef __NX_BSD_DONT_DEFINE_TIMEVAL__
struct timeval
{
...
INT soc_fcntl(INT sockID, UINT flag_type, UINT f_options);
...
VOID _nxd_set_errno(INT tx_errno);
int _nxd_get_errno(void);
/* #define errno (_nxd_get_errno()) */

#define set_errno(e) _nxd_set_errno(e)
